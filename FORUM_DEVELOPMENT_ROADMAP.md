...
- 当前阶段状态更新：
  ...
  - 阶段 2：领域事件、事件帖、资源字段注册层已落地；`ResourceRegistry` 已进一步升级为资源定义 / 关系 include / 扩展字段三层协议，并新增字段/关系预加载声明能力；讨论 / 帖子 / 标签 / 通知 / 搜索 / 用户这些主读接口已开始接入统一 `fields[...]` 与 registry 预加载计划；前端也已开始用应用级单连接消费统一 `forum_event` 协议，并把讨论详情、讨论列表、个人主页、搜索结果页与全局搜索弹窗接到同一套实时资源合并层，通知展示也开始读取最新资源快照，但更完整的 relationships 文档化约定、剩余接口预加载收口、更多事件驱动迁移以及更广覆盖的实时消费仍需继续推进。
