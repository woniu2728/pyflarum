...
- 已完成：前端 `header`、用户菜单和移动抽屉个人入口切换到注册式渲染，前台扩展注入基础继续收口。
- 当前阶段状态更新：
  ...
  - 阶段 1：模块注册中心、权限注册化、后台模块中心基础页、前端基础注入点已落地，`discussion/post` 动作、composer、profile、header 与移动端个人入口已进入注册式渲染，后续重点是继续把更多内置能力从“已注册”推进到“完全按模块接入”。
