...
- 当前阶段状态更新：
  ...
  - 阶段 3：前端 Resource Store 已投入使用；discussion / post Composer 的编辑器、预览、上传、mention / emoji、浮层高度与二级动作运行时也已收口到共享 `useComposerRuntime`，并补上 `registerComposerDraftMeta` / `registerComposerSubmitSuccess` 这两类扩展点，让草稿元信息展示和提交后处理开始从页面硬编码推进到注册式接入；同时新增统一 `usePaginatedListState` 底层，把 discussion list、notification、search 三条页面状态线的首屏加载、路由驱动刷新、分页切换与“加载更多”并发门禁开始收口到同一套列表状态协议，阶段 3 的统一列表/分页状态层已从路线图建议推进到实际落地，后续重点转到更完整的列表状态组件化与详情 / 帖子流状态收口。
