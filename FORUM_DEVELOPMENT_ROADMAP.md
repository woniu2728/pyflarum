...
- 当前阶段状态更新：
  ...
  - 阶段 2：领域事件、事件帖、资源字段注册层已落地；`ResourceRegistry` 已进一步升级为资源定义 / 关系 include / 扩展字段三层协议，并新增字段/关系预加载声明能力；讨论 / 帖子 / 标签 / 通知 / 搜索 / 用户这些主读接口已开始接入统一 `fields[...]` 与 registry 预加载计划，但更完整的 relationships 文档化约定、剩余接口预加载收口和更多事件驱动迁移仍需继续推进。
