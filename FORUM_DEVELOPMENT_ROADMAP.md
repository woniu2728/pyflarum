...
- 当前阶段状态更新：
  - 阶段 0：图标资源本地化已完成；并发回帖修复、分页限制已存在；后台已开始显式暴露并阻止部分 Redis/队列危险配置；认证安全已完成短期 token 收紧、基础安全头、access token 去本地持久化以及 WebSocket 去 URL token，剩余重点转向更完整的 Cookie/CSRF 收口。
