...
- 当前阶段状态更新：
  ...
  - 阶段 2：领域事件、事件帖、资源字段注册层已落地；`ResourceRegistry` 已进一步升级为资源定义 / 关系 include / 扩展字段三层协议，搜索结果、用户详情、讨论与帖子作者摘要已开始按统一资源协议输出，但讨论/帖子/标签/通知主接口距离完整 `fields/include/relationships` 协议仍有继续推进空间，更多事件驱动迁移也仍需继续推进。
