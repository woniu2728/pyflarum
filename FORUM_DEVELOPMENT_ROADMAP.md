...
- 当前阶段状态更新：
  ...
  - 阶段 3：前端 Resource Store 已投入使用；discussion / post Composer 的编辑器、预览、上传、mention / emoji、浮层高度与二级动作运行时也已收口到共享 `useComposerRuntime`，并补上 `registerComposerDraftMeta` / `registerComposerSubmitSuccess` 这两类扩展点，让草稿元信息展示和提交后处理开始从页面硬编码推进到注册式接入；同时新增统一 `usePaginatedListState` 底层，把 discussion list、notification、search 三条页面状态线的首屏加载、路由驱动刷新、分页切换与“加载更多”并发门禁开始收口到同一套列表状态协议；discussion detail 也已进一步拆出 `useDiscussionDetailState` + `useDiscussionPostStreamState` 两层，把讨论资源加载/patch/refresh 与帖子流分页、实时事件消费、本地回复事件合并、阅读进度同步从单一大 composable 中分层抽离，并继续补上统一 `discussion/post` 动作运行时，让详情页菜单、侧栏操作和帖子动作开始通过 registry action + handler map 调度，不再由页面手写 `actionMap` 维护，阶段 3 的详情状态与交互层收口已开始实际落地，后续重点转到更完整的列表状态组件化与详情交互层继续瘦身。
