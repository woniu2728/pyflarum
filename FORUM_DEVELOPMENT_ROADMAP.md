...
- 当前阶段状态更新：
  ...
  - 阶段 3：前端 Resource Store 已投入使用；discussion / post Composer 的编辑器、预览、上传、mention / emoji、浮层高度与二级动作运行时也已收口到共享 `useComposerRuntime`，并补上 `registerComposerDraftMeta` / `registerComposerSubmitSuccess` 这两类扩展点，让草稿元信息展示和提交后处理开始从页面硬编码推进到注册式接入；后续重点转到统一列表/分页状态层与更多模块化提交后编排。
